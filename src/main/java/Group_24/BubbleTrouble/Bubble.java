...
public class Bubble extends Circle {
   ...
   * 
   * @param type
   *          should be a integer holding the collision type, contained by
   *          Collision.[type].
   */
  public void collide(int type) {
    switch (type) {
      case CollisionEvent.TYPE_FLOOR:
        vy = -vy;
        break;
      case CollisionEvent.TYPE_WALL:
        vx = -vx;
        break;
      case CollisionEvent.TYPE_ROPE:
        split();
        break;
      default:
        return;
  ...
  /**
   * Returns whether the other bubble is the same as this bubble.
   */
